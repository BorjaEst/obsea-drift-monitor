...
exclusive to the obsea_drift_monitor package. You can use the `config.py`
...
"""
# Do NOT import anything from `api` or `obsea_drift_monitor` packages here.
...
MODELS_PATH = os.getenv("OBSEA_DRIFT_MONITOR_MODELS_PATH", default=BASE_PATH / "models")
MODELS_PATH = Path(MODELS_PATH)
# Path definition for data folder
DATA_PATH = os.getenv("OBSEA_DRIFT_MONITOR_DATA_PATH", default=BASE_PATH / "data")
DATA_PATH = Path(DATA_PATH)

# configure logging:
# logging level across various modules can be setup via USER_LOG_LEVEL,
# options: DEBUG, INFO(default), WARNING, ERROR, CRITICAL
ENV_LOG_LEVEL = os.getenv("OBSEA_DRIFT_MONITOR_LOG_LEVEL", "INFO")
LOG_LEVEL = getattr(logging, ENV_LOG_LEVEL.upper())

# EXAMPLE on how to load environment variables
PARAMETER_INT = int(os.getenv("OBSEA_DRIFT_MONITOR_PARAMETER_INT", default="10"))
