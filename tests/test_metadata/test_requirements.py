...
def test_authors(metadata):
    ...
    assert metadata["author"] == "Borja Esteban Sanchis"
...
def test_emails(metadata):
    ...
    assert email_author == "Borja Esteban Sanchis"
    email_values = ", ".join(metadata["author-email"].values())
    assert email_values == "boressan@outlook.com"


def test_description(metadata):
    """Tests that metadata provides description information."""
    assert "description" in metadata
    assert "Drift detector and monitor for obsea data adquisition." in metadata["description"]
